...
class SystemGenerator {
   ...
   *
   * Replace refercnes to 'torqueborg', 'TorqueBorg', and 'BOILERPLATE'
  ...
  replaceFileContents() {
    ...
    const replacements = [
      {
        pattern: new RegExp(/game\.torqueborg/g),
        replacement: `game.${this.propName}`
      },
      {
        pattern: new RegExp(/flags\.torqueborg/g),
   ...
   * Rename files.
   *
   * Rename files that had torqueborg in their name, such as
   * css/torqueborg.css.
   */
  renameFiles() {
    glob(`build/${this.packageName}/**/*torqueborg*.*`).then(files => {
      files.forEach(oldPath => {
        const file = path.basename(oldPath);
        const directory = path.dirname(oldPath);
        fs.rename(oldPath, `${directory}/${file.replaceAll('torqueborg', this.packageName)}`, (err) => {
          if (err) throw err;
        });
      })
    })
  ...
  cleanPackageJson() {
    ...
    fs.rmSync(`build/${this.packageName}/src/generate-torqueborg-system.mjs`);
...
inquirer
  ...
  .then((answers) => {
    ...
    // Glob TorqueBorg's files so that we can process them.
    glob('*', {ignore: ['node_modules/**'] }).then(files => {
      // Copy all files into the build dir.
      generator.copyFiles(files);
      // Replace torqueborg name mentions in files.
      generator.replaceFileContents();
      // Rename files that had torqueborg in their name.
